...
class TiKAN(nn.Module):
    ...
    def forward(self, x):
        ...
        zx = zx.view(zx.size(0), self.input_steps*self.latent_dim)
        z_pred = self.kan(zx)
        z_pred = z_pred.view(z_pred.size(0), self.input_steps+self.forecast_steps, self.latent_dim)
        pred = self.autoencoder.decode(z_pred)
        return pred, z_pred
